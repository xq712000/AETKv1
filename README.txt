...
*                                                                            		 	*
* 4) allSpeedUnsteadyFoam installation                                    	*
*                                                                             			*
* 5) case testing				                                    	*
*                                                                            			*
* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
...
3) OpenFOAM installation
  ...
  Skip this step if you have a functionally version of OpenFOAM-2.0.1
  already installed on your pc.

Download the source code http://www.openfoam.org/archive/2.0.1/download/source.html
  

Unpacking the Sources

The user should choose a directory location to unpack these files, which will become the 
installation directory of OpenFOAM. I established a folder named 'OpenFOAM' in the 
directionary '/home/username/' and put the source pack in this folder' /home/username/OpenFOAM'. 

After the installation directory is chosen (and, if necessary, created), simply copy the 2 source pack files 
into the directory and unpack using tar xzf <filename>, e.g. from the installation directory:
...
several packages need to be installed. This is a command line that will install most of the necessary 
(and some possibly unnecessary) packages for building OpenFOAM and ParaView:
...
-------------------------------------------------------------------------------
4) allSpeedUnsteadFoam and allSpeedUnsteadyFoam_dualtime installation

   allSpeedUnsteadFoam is a collect of additional library for OpenFOAM.
   
 Two solvers are included, allSpeedUnsteadFoam and allSpeedUnsteadyFoam_dualtime. 
The dual time step scheme is implemented in the second solver allSpeedUnsteadyFoam_dualtime 
which is based on the allSpeedUnsteadFoam.
...
Note: in the text bellow, 'temp' denotes the temporary directory(i.e.  /home/username/mysolver).
 'OF' denotes the root directory of 'OpenFOAM', ("/home/username/OpenFOAM/OpenFOAM-2.0.1" in my system.)

  ** Compiling the source

(1) copy the timeStepping /MRF* files to OF/src/finiteVolume/cfdTools/general/MRF/

'cp temp/timeStepping /MRF  OF/src/finiteVolume/cfdTools/general/MRF/'

and recompile the lib finiteVolume

'cd OF/src/finiteVolume' and then 'wmake libso'

Note: Here, 'MRF' refers to 'Moving Reference Frame'. This folder contains the geometric information about MRF zone.

(2)Enter the folder timeStepping by typing in a terminal
...
(3)Enter the folder godunovFlux by typing in a terminal
...
(4)Enter the folder inv by typing in a terminal
...
(5)Enter the folder allSpeedUnsteadyFoam by typing in a terminal
...
'wmake'
(6)Enter the folder allSpeedUnsteadyFoam_dualtime by typing in a terminal
...
Note: at every step above, if compile successfully, the words' ¡­ is up to date .'
-------------------------------------------------------------------------------
5) case testing	
