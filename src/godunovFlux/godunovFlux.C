...
Description
    Generic Godunov flux class. It is modified based on Godunov flux class 
	in densityBasedTurbo solver developed by  Oliver Borm. 
...
//// Refer to section 3.3. Implementation of interface reconstruction in the paper
    pos
    (
        IOobject
        (
...
// add  2013.7.18
    const volScalarField& Urref2 =
        mesh_.lookupObject<volScalarField>("Ur2");
...
//// Refer to section 3.3. Implementation of interface reconstruction in the paper
         ...
         (
             fvc::interpolate(p_, pos, "reconstruct(p)")
         );
         surfaceScalarField p_neg
         (
             fvc::interpolate(p_, neg, "reconstruct(p)")
         );

         surfaceVectorField U_pos
         (
             fvc::interpolate(U_, pos, "reconstruct(U)")
         );
         surfaceVectorField U_neg
         (
             fvc::interpolate(U_, neg, "reconstruct(U)")
         );

         surfaceScalarField rho_pos
         (
             fvc::interpolate(rho_, pos, "reconstruct(rho)")
         );
         surfaceScalarField rho_neg
         (
             fvc::interpolate(rho_, neg, "reconstruct(rho)")
         );

         surfaceScalarField k_pos
         (
             fvc::interpolate(k_, pos, "reconstruct(k)")
         );
         surfaceScalarField k_neg
         (
             fvc::interpolate(k_, neg, "reconstruct(k)")
         );
////////////////////////////////////////////////////////////////////////////////////////
////////////////////////////////////////////////////////////////////////////////////////////
///// The code between "/*" and  "*/" is the old content about limiter factor.  
/* 
    if (secondOrder)
    {
...
//// Refer to section 3.3. Implementation of interface reconstruction in the paper
		const fvPatchScalarField& pUrref2= Urref2.boundaryField()[patchi];
	...
	const fvsPatchScalarField& prho_neg = rho_neg.boundaryField()[patchi];
        ...
        const fvsPatchScalarField& pk_pos = k_pos.boundaryField()[patchi];
	const fvsPatchScalarField& pk_neg = k_neg.boundaryField()[patchi];
...
// add 2013.7.18
			const scalarField pUrref2Left = pUrref2.patchInternalField();
			const scalarField pUrref2Right = pUrref2.patchNeighbourField();	
...
//                    pkRight[facei] + secondOrder*pkLimiterRight[facei]*(pGradkRight[facei] & deltaRRight),               // face T
                    ...
                    prho_pos[facei]  ,                 // face T
                    prho_neg[facei] ,               // face T
